...
#{
    ...
    #{
        ...
        #{
            executable_code = i + ' = '+ i + '.select(' + str(df_source[i]["sourceschema"]) + ')'
            #print(executable_code)
            outF.write('\t' + executable_code + '\n')
...
#{
 ...
 #empDF.join(deptDF,empDF.emp_dept_id ==  deptDF.dept_id,"inner").show(truncate=False)  
    ...
    c = ["dataframe","source_column","target_column","transformation_operation","transformation_operator","expression","params","value","value_type","datatype","filter_type","list_of_columns","left_dataframe","right_dataframe","join_type","join_condition","resultant_dataframe"]
    ...
    df_transformation = pd.DataFrame(data['transformation'], columns = c)
    
    df_transformation = df_transformation.fillna('None')
    
    for i in data['source']:
    
        availableSourceDF.append(i)
        
    
    print(availableSourceDF)
    
    #print(data['transformation'][0])
    #print(df_transformation.columns)
...
def func_partition(df_t,fileType,partCols,path,maxRecords,TargetSchema):
 #{   
    global outF
    
    for cols in partCols:
        
        executable_code=""
        executable_code = df_t+' = '+df_t+'.withColumn("col_"+"'+cols+'",'+df_t+'["'+cols+'"])'
        
        
        outF.write('\t' + executable_code + '\n')
    
    outF.write('\t'+df_t+'.show()\n')
    ...
    if len(TargetSchema) != 0:
        ...
        executable_code = df_t + ' = '+ df_t + '.select(' + str(TargetSchema) + ')'
        
        outF.write('\t' + executable_code + '\n')
                    
    executable_code=""
    executable_code = 'reParts = '+df_t+'.count() / int('+str(maxRecords)+')'
    outF.write('\t' + executable_code + '\n')
    
    executable_code=""
    executable_code = 'reParts = math.ceil(reParts)'
    outF.write('\t' + executable_code + '\n')
    
    
    executable_code = 'if reParts <= 1 :'
    outF.write('\t' + executable_code + '\n')
    
    executable_code=""
    executable_code=df_t+'.write.partitionBy('+str(partCols)+').mode("overwrite").'+fileType+'("'+path+'")'
    
    outF.write('\t\t' + executable_code + '\n')
    
    
    executable_code=""
    executable_code = 'else:'
    outF.write('\t' + executable_code + '\n')
    ...
    #{
        ...
        #{  
                ...
                if len(df_target[i]["targetschema"]) != 0:
                    ...
                    executable_code = i + ' = '+ i + '.select(' + str(df_target[i]["targetschema"]) + ')'
